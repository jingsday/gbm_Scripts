---
title: "gbm_Scripts_cgga_bind"
author: "Jing"
date: "2025-01-17"
output: html_document
conclusion: not working with cgga. Dunno why
---
...
#clin 693 and clin 325 primary pacients had been treated before they were sampled.
#To fit our purpose, further filtering should be performed
table(clin_325$Radio_status..treated.1.un.treated.0.)
table(clin_693$Radio_status..treated.1.un.treated.0.)
...
clin_693 <- clin_693[clin_693$Radio_status..treated.1.un.treated.0. %in% c('0'),]
clin_693 <- clin_693[clin_693$Chemo_status..TMZ.treated.1.un.treated.0. %in% c('0'),]
...
surv_filt <-surv_filt[-c(117 , 135 , 137 , 141),]
RNA_combined_filt <- RNA_combined_filt[-c(117 , 135 , 137 , 141),]
comb_clin_filt<-comb_clin_filt[-c(117 , 135 , 137 , 141),]
path_filt <-path_df[-c(117 , 135 , 137 , 141),]
...
print(paste(top_100$variable,collapse=" + "))
# if we want to run coxph we have to copy-paste the string
fit.coxph <- coxph(surv_filt ~ CSNK1A1 + NMT1 + TXNL4B + MLEC + AXIN1 + PLA2G15 + SMC4 + FCHO1 + EGF + FOXJ3 + COL4A1 + TES + SIRT3 + CSK + HSPB1, data = RNA_combined_filt)
ggforest(fit.coxph, data = RNA_combined_filt)
