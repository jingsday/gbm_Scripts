...
print(paste("Median Overall Survival:", median_surv, "Days"))

plot_title <- paste("KM Survival Curve Median OS:", round(median_surv,1), "Days")
...
clin$Gender <- trimws(clin$Gender)
...
ggsurvplot(fit, data = clin_filt, xlab = "Days", ylab = "Overall survival")
which(is.na(surv_filt))
paste(which(is.na(surv_filt)),collapse = ' , ')

RNA_filt[is.na(RNA_filt)] <- 0

surv_filt <-surv_filt[-c(69 , 75 , 96),]
RNA_filt <- RNA_filt[-c(69 , 75 , 96),]

clin_filt<-clin_filt[-c(69 , 75 , 96),]
...
fit.coxph <- coxph(surv_filt ~ SPDEF + LYN + DFFA + KIT + PRSS23 + NMT1 + ARL4C + SIRT3 + ALDH7A1 + MEST + LAMA3 + INPP4B + RRP1B + SERPINE1 + ABCF1, data = RNA_filt)
...
# Plot Kaplan Meier 
#plot(KM, lwd = 3, col = c(1,2), cex.axis = 1.5, xlab = 'Days', ylab = 'Survival Probability' , cex.lab = 1.5)
#ggsurvplot(KM, data = path_filt,pval = TRUE,xlab = 'Overall survival time, Days',
...
B <- 1000
# Building a RSF
status_surv <- clin_filt$Censor..alive.0..dead.1.=="1"
time_surv <- clin_filt$OS
